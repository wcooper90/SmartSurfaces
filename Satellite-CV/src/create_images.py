...
def create_images(areas, path):
    ...
    for i in range(length):
        ...
        try:
            ...
            img = urllib.request.urlretrieve("http://maps.googleapis.com/maps/api/staticmap?center=" + str(areas[i][0]) + ","+ str(areas[i][1]) + "&zoom=19&size=640x640&sensor=false&maptype=satellite&key=#######&v=3", path + str(i) + ".PNG")
